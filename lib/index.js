...
var dropUtil = require('../utils/dropUtil');
    ...
    input.parseOptions(optionsRaw, callbackRaw, function (options, callback) {
      ...
      file.openTemplate(templatePath, function (err, template) {
        ...
        preprocessor.execute(template, options, function (err, template) {
          ...
          // 提取base64图片
          let base64Images = imageUtil.getBase64ImagesFromData(data);
          // 处理data的空格为可显示的,默认空格会被去掉,我的业务场景需要展示空格
          textUtil.spaceObject(data);
          // 处理纯数组[1,2]为对象[{value:1},{value:2}],方便渲染纯数组
          textUtil.arrayObject(data);
          // parse all files of the template
          walkFiles(template, data, options, 0, function (err, report) {
            if (err) {
              return callback(err, null);
            }
            postprocessor.process(template, data, options); // Post-process the template

            if (template.extension == 'docx') {
              // 执行删除元素操作
              dropUtil.dropElementToDocument(report);
              imageUtil.putImagesToDocument(report, base64Images, () => {
                // 整体处理处理文字   字体 、加粗、大小
                textUtil.putFontStyleFullToDocument(report, optionsRaw);
                // 局部处理处理文字   字体 、加粗、大小、颜色
                textUtil.putFontStyleToDocument(report);
                postprocessor.process(template, data, options); // Post-process the template
              ...
              // 执行删除元素操作
              dropUtil.dropElementToExcel(report);
              // assemble all files and zip if necessary
              file.buildFile(report, function (err, result) {
                if (err) {
                  return callback(err, null);
...
carbone.addFormatters(require('../formatters/custom.js'));
translator.loadTranslations(params.templatePath);
